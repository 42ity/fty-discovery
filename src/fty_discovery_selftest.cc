/*  =========================================================================
    fty_discovery_selftest.c - run selftests
...
// Tests for stable public classes:
    { "fty_discovery_server", fty_discovery_server_test, true, true, NULL },
...
{
    test_item_t *item;
    printf ("Running fty-discovery selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_DISCOVERY_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_discovery_private_selftest (verbose, item->subtest);
#endif // FTY_DISCOVERY_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_discovery_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-discovery test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_DISCOVERY_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_discovery_private_selftest (verbose, test->subtest);
#endif // FTY_DISCOVERY_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
