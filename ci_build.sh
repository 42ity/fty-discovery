...
default|default-Werror|default-with-docs|valgrind|clang-format-check)
    ...
    if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libfty-asset-dev >/dev/null 2>&1) || \
    ...
    ; then
        ...
        cd ./tmp-deps
        $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/fty-asset.git fty-asset
        cd ./fty-asset
        CCACHE_BASEDIR=${PWD}
        export CCACHE_BASEDIR
        git --no-pager log --oneline -n1
    ...
    # Start of recipe for dependency: fty-asset-accessor
    if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libfty-asset-accessor-dev >/dev/null 2>&1) || \
           (command -v brew >/dev/null 2>&1 && brew ls --versions fty-asset-accessor >/dev/null 2>&1) \
    ; then
        echo ""
        BASE_PWD=${PWD}
        echo "`date`: INFO: Building prerequisite 'fty-asset-accessor' from Git repository..." >&2
        cd ./tmp-deps
        $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/fty-asset.git fty-asset-accessor
        cd ./fty-asset-accessor
        CCACHE_BASEDIR=${PWD}
        export CCACHE_BASEDIR
        git --no-pager log --oneline -n1
        if [ -e autogen.sh ]; then
    ...
    # Start of recipe for dependency: fty-utils
    if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libfty-utils-dev >/dev/null 2>&1) || \
           (command -v brew >/dev/null 2>&1 && brew ls --versions fty-utils >/dev/null 2>&1) \
    ; then
        echo ""
        BASE_PWD=${PWD}
        echo "`date`: INFO: Building prerequisite 'fty-utils' from Git repository..." >&2
        cd ./tmp-deps
        $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/fty-utils.git fty-utils
        cd ./fty-utils
        CCACHE_BASEDIR=${PWD}
        export CCACHE_BASEDIR
        git --no-pager log --oneline -n1
